...
public class CompletionProvider {
    ...
    private void populateItemsFromImportNode(ImportNode importNode, Position position, List<CompletionItem> items) {
		...
		List<CompletionItem> localClassItems = ast.getClassNodes().stream().filter(classNode -> {
			...
			if (packageName == null || packageName.length() == 0 || (enclosingModule != null && packageName.equals(enclosingModule.getPackageName()))) {
                return false;
            }
			...
			if (enclosingModule != null && enclosingModule.getImports().stream()
				.map(otherImportNode -> otherImportNode.getClassName()).collect(Collectors.toList()).contains(className)) {
				return false;
			}
			return true;
		}).map(classNode -> {
